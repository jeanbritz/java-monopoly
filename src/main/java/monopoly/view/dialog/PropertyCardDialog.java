...
import monopoly.model.persistent.Tariff;
...
public class PropertyCardDialog extends AbstractDialog<Property> {
	...
	private JTable tableTariffs;
	private JLabel labelSmallNote;
	private JLabel labelHouseCost;
	private JLabel labelMortage;
	...
	public void setData(Property data) {
		...
		((CenteredLabel) labelMortage).setText("MORTGAGE value of site, R " + data.getPMortageVal());
	...
	private void setEstatePropertyData(Property data) {
		...
		((CenteredLabel) labelSmallNote).setText("If a player owns ALL the Sites of any Colour-Group,"
		    + " the rent is Doubled on Unimproved Sites in that group");

		Tariff oneHouse = data.getTariffs().get(1);
		((CenteredLabel) labelHouseCost).setText("COST of Houses, R " + oneHouse.getTCost()
		    + " each <br> COST of Hotels, R " + oneHouse.getTCost() * 5 + " each");
	...
	private void setBoardPropertyData(Property data) {
		...
		((CenteredLabel) labelSmallNote).setText("");
		((CenteredLabel) labelHouseCost).setText("If one <em>Utility</em> is owned, rent is 4 times amount shown on dice."
	...
	public void initView() {
		...
		labelMortage = new CenteredLabel("Mortgage", null);
